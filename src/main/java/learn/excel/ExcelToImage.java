 ...
 * @author https://www.hexcode.cn/(丁丁的博客)
...
public class ExcelToImage {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Workbook wb = WorkbookFactory.create(new File("D:\\研发中心.xlsx"));
        Sheet sheet = wb.getSheet("研发中心");
        
        //获取合并单元格的数量并将其放入事先准备好的集合里面
        int mergedRegionCounts = sheet.getNumMergedRegions();
        List<CellRangeAddress> rangeAddress = new ArrayList<CellRangeAddress>();//sheet.getgetMergedRegions(); // 获取整个sheet中合并单元格组合的集合
        for(int i=0;i<mergedRegionCounts; i++){
          rangeAddress.add(sheet.getMergedRegion(i));
        }
        ...
        for (int i = 0; i < rowSize; i++) {
          ...
          for (int j = 0; j < colSize; j++) {
            ...
            cells[i][j].setCell(row.getCell(j));
            cells[i][j].setRow(i);
            cells[i][j].setCol(j);
          ...
          ifShow=ifShow && !row.getZeroHeight();  //行序列不能隐藏
          float heightPoint = sheet.getRow(i).getHeightInPoints(); // 如果该单元格是隐藏的，则置高度为0
          imageHeight += heightPoint;        
          rowPixPos[i+1] = (int) (heightPoint * 96 / 72) + rowPixPos[i];//原代码 rowPixPos[i+1] = (int) (heightPoint * 96 / 72) + rowPixPos[i]
 
        }
 
        imageHeight = imageHeight * 96 /72;//imageHeight = imageHeight * 96/72
        ...
        g2d.dispose();
        ImageIO.write(image, "png", new File("d:/研发中心.png"));
